...
public class ProductRequestService {
  ...
  public List<ProductRequest> getRequestListByProductAndPrice(Product product, Double maxPrice, Long userId) {
    return em.createQuery("SELECT pr " +
        ...
        "ORDER BY pr.price", ProductRequest.class).getResultList();
  }


  public boolean buyProduct(Balance balance, Users user, ProductStore userStorage, Product product, Long quantity, Double price) {
    List<ProductRequest> tmpProductList = getRequestListByProductAndPrice(product, price, user.getId());
    ...
    for (ProductRequest productRequest : requestsToBuy) {
      if ( productRequest.getQuantity() < quantity) {
        ...
        double currentPrice = productRequest.getPrice() * quantity;
        finalPrice += currentPrice;
        balanceService.changeBalance(currentPrice, productRequest.seller);
